...
def facial_stroke_prediction_model(image):
    ...
    facial_model = efficientnet_b0(weights=None)
    # Replace the final classification layer
    facial_model.classifier[1] = torch.nn.Linear(in_features=1280, out_features=2)
    # Load the pretrained weights
    facial_model.load_state_dict(torch.load('/home/mich02/Desktop/UMKC_DS_Capstone/model_pretrained/facial_stroke_model.pth', map_location=torch.device('cpu')))
    facial_model.eval()
...
def combined_stroke_probability(image, gender, age, hypertension, heart_disease, avg_glucose_level, bmi, smoking_status):
    ...
    return f"Stroke Probability: {combined_probability:.2%}"
